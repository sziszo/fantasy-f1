...
# docker
DOCKER_FANTASY_F1_NETWORK = 'fantasy_f1'

# kubernetes
FANTASY_F1_NAMESPACE = 'fantasy-f1-ns'
INFRA_DB_CONFIG = 'infra-db-config'
...
class DCInfraManager(InfraManager):
    ...
    def create_network(self):
        try:
            ...
            self.dockerClient.networks.get(DOCKER_FANTASY_F1_NETWORK)
            print('ok!')
        except NotFound:
            print('failed!')
            print("creating network for todo-apps", end='')
            self.dockerClient.networks.create(DOCKER_FANTASY_F1_NETWORK)
            print('done!')
...
class K8SInfraManager(InfraManager):
    ...
    def __create_namespace(self):
        print(f'checking {FANTASY_F1_NAMESPACE} namespace settings... ', end='')
        res = self.coreApi.list_namespace(field_selector=f'metadata.name={FANTASY_F1_NAMESPACE}')
        if not len(res.items):
            print('failed!')
            print("creating namespace for todo-apps", end='')
            ns = k8s_client.V1Namespace(metadata=k8s_client.V1ObjectMeta(name=FANTASY_F1_NAMESPACE))
    ...
    def __create_infra_db_config_map(self):
        config_data = self.read_dot_env_file(ENV_FILE)
    ...
    def __create_config_map(self, name, config_data=None, config_files=None):
        ...
        res = self.coreApi.list_namespaced_config_map(namespace=FANTASY_F1_NAMESPACE,
        ...
        if not len(res.items):
            ...
            if config_data:
                self.coreApi.create_namespaced_config_map(namespace=FANTASY_F1_NAMESPACE, body=config_map)
            elif config_files:
                command = ['kubectl', 'create', 'configmap', name, '--namespace', FANTASY_F1_NAMESPACE]
    ...
    def start(self):
        self.setup()
        res = self.appsApi.list_namespaced_deployment(namespace=FANTASY_F1_NAMESPACE,
        ...
        if len(res.items):
            ...
            create_from_yaml(k8s_client=k8s_client.ApiClient(), yaml_file=f'{INFRA_DIR}/{self.INFRA_YAML}',
                             namespace=FANTASY_F1_NAMESPACE)
    ...
    def __delete_deployment(self, name):
        try:
            ...
            self.appsApi.delete_namespaced_deployment(namespace=FANTASY_F1_NAMESPACE, name=name)
            print('done!')
        except ApiException:
            print('failed!')
    ...
    def __delete_service(self, name):
        try:
            ...
            self.coreApi.delete_namespaced_service(namespace=FANTASY_F1_NAMESPACE, name=name)
            print('done!')
        except ApiException:
            print('failed!')
    ...
    def __delete_config_map(self, name):
        try:
            ...
            self.coreApi.delete_namespaced_config_map(namespace=FANTASY_F1_NAMESPACE, name=name)
            print('done!')
        except ApiException:
            print('failed!')
